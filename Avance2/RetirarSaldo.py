...
def depositarDinero(autenticado):
    ...
    # Cargar valores de conversión desde el archivo
    conversorDivisas = {}
    try:
        with open("conversor_divisas.txt", "r") as conversor_file:
            for line in conversor_file:
                moneda, valor = line.strip().split(":")
                conversorDivisas[moneda] = float(valor)
    except FileNotFoundError:
        print("Archivo de conversión de divisas no encontrado. Se usarán valores por defecto.")
        conversorDivisas = {"COL": 520, "BTC": 0.000034, "USD": 1.0}
    ...
    # Guardar valores de conversión en el archivo
